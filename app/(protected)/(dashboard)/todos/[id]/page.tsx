...
import { ArrowLeft, Calendar, Save, Lock, User, Check, Tag, FileText, Loader2, CalendarDays, Network } from "lucide-react";
import { CategoryCombobox } from "@/components/todos/CategoryCombobox";
import MindMapCanvas from "@/components/todos/MindMapCanvas";
...
const TodoDetailPage = () => {
    ...
    const [activeTab, setActiveTab] = useState<'document' | 'mindmap'>('document');
    ...
    const fetchData = React.useCallback(async () => {
        ...
        if (todoError) {
            ...
            if (isSafeToUpdate) {
                ...
                const serverMindMap = todoData.mind_map || null;
                ...
                setMindMapData((prev: any) => {
                    // Simple check: if server has data and we have nothing, take server.
                    // If we have data, we assume local is fresher due to auto-save guard.
                    // Ideally we need deep comparison or timestamp check, but for now:
                    if (!prev && serverMindMap) return serverMindMap;
                    return prev;
                });
    ...
    useEffect(() => {
        ...
        const isMindMapChanged = JSON.stringify(mindMapData) !== JSON.stringify(todo.mind_map || null);

        if (!isTitleChanged && !isContentChanged && !isDescChanged && !isMindMapChanged) return;
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (!error) {
                    ...
                    setTodo(prev => prev ? ({ ...prev, title: localTitle, content, description, mind_map: mindMapData }) : null);
                }
            } catch (err) {
    ...
    }, [localTitle, content, description, mindMapData, todo, id, supabase, loading, canEdit]);
    ...
    }, [todo, canEdit, saving, localTitle, content, description, mindMapData, id, supabase]);
    ...
    const handleMindMapChange = React.useCallback((data: any) => {
        setMindMapData(data);
        lastLocalEdit.current = Date.now();
    }, []);
            ...
            <div className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 shrink-0 z-50">
                ...
                <div className="flex items-center w-[200px]">
                    <button
                        ...
                        className="flex items-center text-slate-500 hover:text-slate-800 transition-colors gap-1 px-2 py-1 rounded-lg hover:bg-slate-100"
                    >
                        <ArrowLeft size={18} />
                ...
                <div className="flex-1 flex justify-center">
                    <div className="flex bg-slate-100 p-1 rounded-lg">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'document' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            <FileText size={14} /> 문서
                        </button>
                        <button
                            onClick={() => setActiveTab('mindmap')}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${activeTab === 'mindmap' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                ...
                {/* Right: Actions */}
                <div className="flex items-center justify-end gap-3 text-xs font-medium text-slate-400 w-[200px]">
