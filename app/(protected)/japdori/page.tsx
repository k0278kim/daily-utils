    ...
    <AnimatePresence>
      {
        addJapdoriOverlay && <motion.div className={`fixed flex w-full h-full items-center justify-center bg-black/20 z-20 duration-1000 ${addJapdoriOverlay ? "backdrop-blur-xl" : ""}`} transition={roundTransition} exit={{ opacity: 0 }}>
          <AddJapdoriOverlay setJapdories={setJapdories} japdoriFromEmail={user?.email} setAddJapdoriOverlay={setAddJapdoriOverlay} me={me} setMe={setMe} />
        </motion.div>
      }
    </AnimatePresence>
...
const UserBlock = ({ user, selectedUser, setSelectedUser, japdoriesNumber }: userBlockType) => {
  return <motion.div
    ...
    <div className={`duration-100 w-7 h-7 flex items-center justify-center text-white opacity-70 font-bold rounded-full ${japdoriesNumber > 0 ? "bg-red-500" : selectedUser?.id == user.id ? "bg-gray-800" : "bg-gray-700"}`}>{japdoriesNumber}</div>
  </motion.div>
}
